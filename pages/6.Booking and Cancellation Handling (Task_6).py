...
def flight_booking():
    ...
    if source_city == destination_city:
        ...
        return_date = st.date_input("Return Date (optional)", min_value=departure_date, value=st.session_state.flight_return, key="flight_return")
        travel_class = st.selectbox("Select Class", ["Business", "First"], key="flight_class")

        if st.button("Search Flights"):
            with st.spinner("Fetching flight data..."):
                flight_data = get_flight_offers(city_iata_mapping[source_city], city_iata_mapping[destination_city], departure_date.strftime('%Y-%m-%d'), return_date.strftime('%Y-%m-%d') if return_date else None)
                if flight_data:
...
def hotel_booking():
    ...
    city = st.selectbox("Select City", ["Mumbai", "Delhi", "Bangalore", "Hyderabad", "Chennai"], index=0 if st.session_state.selected_city is None else ["Mumbai", "Delhi", "Bangalore", "Hyderabad", "Chennai"].index(st.session_state.selected_city), key="hotel_city")
